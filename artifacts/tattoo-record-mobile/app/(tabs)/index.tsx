...
function AuthScreen() {
  ...
  return <KeyboardAvoidingView behavior="padding" style={[styles.authRoot, { backgroundColor: colors.background }]}>
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[styles.authContent, { paddingTop: insets.top + 24, paddingBottom: insets.bottom + 24 }]}>
...
function CreateView({ token, user }: { token: string; user: User }) {
  ...
  const pick = async (camera = false) => {
    const result = camera ? await ImagePicker.launchCameraAsync({ mediaTypes: ["images"], quality: 0.9 }) : await ImagePicker.launchImageLibraryAsync({ mediaTypes: ["images", "videos"], quality: 0.9 });
...
function AppShell() {
  ...
  const content = view === "home" ? <HomeView token={token} user={user} openProfile={openProfile} /> : view === "discover" ? <DiscoverView token={token} openProfile={openProfile} /> : view === "create" ? <CreateView token={token} user={user} /> : view === "inbox" ? <InboxView token={token} user={user} /> : view === "profile" ? <ProfileView token={token} user={user} profile={profile || user} onBack={profile ? () => { setProfile(null); setView("home"); } : undefined} openProfile={openProfile} onSignOut={signOut} /> : view === "settings" ? <SettingsView token={token} user={user} onBack={() => setView("profile")} onSignOut={signOut} /> : <ListView kind={view} token={token} user={user} onBack={() => setView("profile")} />;
...
const styles = StyleSheet.create({
  ...
  bottomNav: { position: "absolute", left: 0, right: 0, bottom: 0, minHeight: 74, borderTopWidth: 1, flexDirection: "row", paddingBottom: Platform.OS === "web" ? 34 : 4 },
