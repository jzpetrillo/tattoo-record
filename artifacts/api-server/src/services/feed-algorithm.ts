import { db } from "../db";
import { publicUserColumns } from "../storage";
import { posts, follows, postLikes, savedPosts, users } from "@workspace/db";
...
export async function getFeaturedPosts(limit: number = 20, userId?: string) {
  return db
    ...
    .innerJoin(users, eq(posts.authorId, users.id))
    .where(
      and(isNull(posts.deletedAt), eq(posts.visibility, "PUBLIC"), eq(posts.isFeatured, true))
    )
...
export async function getForYouRecommendations(userId: string, limit: number = 10) {
  ...
  // 2. Find "similar users" — users followed by the same people the current user follows
  ...
  // 3. Get suggested users with public columns only
  let suggestedUsers: any[] = [];
  if (recommendedUserIds.length > 0) {
    const userResults = await db
      .select({ user: publicUserColumns })
      .from(users)
      .where(and(inArray(users.id, recommendedUserIds), eq(users.isVerified, true)))
      .limit(limit);
    ...
    for (const { user } of userResults) {
      ...
      suggestedUsers.push({ user, followersCount: countResult?.count || 0, mutualFollows: 0 });
    }
  }

  // 4. Fall back to popular verified artists/studios if not enough
